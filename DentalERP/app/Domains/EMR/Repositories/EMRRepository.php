...
final class EMRRepository implements EMRRepositoryInterface
{
    ...
    {
        $query = EMR::with(['patient', 'doctor', 'appointment'])->where('organization_id', $filters['organization_id']);
    ...
    {
        return EMR::with(['patient', 'doctor', 'appointment'])->where('id', $id)->where('organization_id', $organizationId)->first();
    }
    ...
    {
        $emr->update($data);
        return $emr->refresh()->load(['patient', 'doctor', 'appointment']);
    }
