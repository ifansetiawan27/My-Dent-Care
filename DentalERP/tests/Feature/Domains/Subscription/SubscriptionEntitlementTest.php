...
test('active organization has full access', function () {
    expect($this->entitlement->isAccessRestricted($this->orgId))->toBeFalse();
    expect($this->entitlement->getAccessLevel($this->orgId))->toBe('full');
});

test('expired organization has restricted access', function () {
    $this->sub->update(['status' => SubscriptionStatus::Expired]);
    expect($this->entitlement->isAccessRestricted($this->orgId))->toBeTrue();
    expect($this->entitlement->getAccessLevel($this->orgId))->toBe('restricted');
});
...
test('nonexistent organization is not restricted', function () {
    expect($this->entitlement->isAccessRestricted((string) Str::orderedUuid()))->toBeFalse();
});
